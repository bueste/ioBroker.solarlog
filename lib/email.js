...
const { formatSwissDate, formatGermanPeriodLabel } = require('./dates');
...
/**
 * @param {{fromDate: string, toDate: string, label: string}} period ISO dates/label, e.g.
 *   from lib/scheduling.js determineScheduledPeriod() - converted to Swiss display format
 *   here (DD-MM-YYYY / "August 2026") since this is the only place these values reach a
 *   human reader (a tenant's inbox), not internal storage.
...
function buildReportEmailContent(period, subjectTemplate, bodyTemplate) {
    const values = {
        periode: formatGermanPeriodLabel(period.label),
        von: formatSwissDate(period.fromDate),
        bis: formatSwissDate(period.toDate),
    };
    return {
