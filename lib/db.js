...
// Optional, per-apartment fixed monthly cost(s) (e.g. building maintenance, a Zaehler-
// gebuehr, a common-area share) that ride alongside the energy-based Solarbezug/
// Netzbezug billing but are not themselves energy-based - flat CHF amounts per meter per
// month. "bezeichnung" allows MULTIPLE distinct named cost items for the same meter/
// month (e.g. "Zaehlerkosten" 5.00 CHF AND "Allgemeinstrom-Anteil" 12.00 CHF side by
// side) - a single combined amount per meter/month was not enough (real-world billing
// needs itemized lines a tenant can actually verify against their own records).
...
async function ensureSchema(pool) {
    ...
    // meter_umlagekosten: add "bezeichnung" (itemized cost label) and widen the primary
    // key to include it, so a meter/month can have several distinct named cost lines
    // instead of just one combined amount. "ADD COLUMN IF NOT EXISTS" can't also swap a
    // PRIMARY KEY conditionally in one go, so this checks first and only runs the
    // (column + PK) migration once, ever - re-running "DROP PRIMARY KEY, ADD PRIMARY KEY"
    // against an already-migrated table would needlessly fail.
    const [bezeichnungCol] = await pool.query(
        `SELECT COUNT(*) AS cnt FROM information_schema.COLUMNS
         WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'meter_umlagekosten' AND COLUMN_NAME = 'bezeichnung'`,
    );
    if (Number(bezeichnungCol.cnt) === 0) {
        await pool.query(
            `ALTER TABLE meter_umlagekosten
             ADD COLUMN bezeichnung VARCHAR(100) NOT NULL DEFAULT 'Umlagekosten' AFTER meter_name,
             DROP PRIMARY KEY,
             ADD PRIMARY KEY (reading_year, reading_month, meter_name, bezeichnung)`,
        );
    }
} // END ensureSchema
...
/**
 * year=0, month=0 is a deliberate sentinel meaning "standing default for this meter +
 * Bezeichnung, applies to every month" (mirrors how Tarif.default is the fallback used
 * whenever no Tarif.<year>.<month> override exists) - both columns already accept 0 with
 * no schema change, and it can never collide with a real calendar month/year. Callers
 * that mean an actual month must still validate 1-12 themselves; this function itself
 * stays agnostic so the sentinel and real months share one upsert path.
 *
 * @param {import('mariadb').Pool} pool
 * @param {number} year 0 = standing default, otherwise a real calendar year
 * @param {number} month 0 = standing default, otherwise 1-12
 * @param {string} meterName
 * @param {string} bezeichnung item label, e.g. "Zaehlerkosten" - part of the primary key,
 *   so the same meter/month (or the default) can carry several distinct named cost lines
 * @param {number} umlagekostenChf
 * @param {boolean} active
 */
async function upsertMeterUmlagekosten(pool, year, month, meterName, bezeichnung, umlagekostenChf, active) {
    await pool.query(
        `INSERT INTO meter_umlagekosten (reading_year, reading_month, meter_name, bezeichnung, umlagekosten_chf, active)
         VALUES (?, ?, ?, ?, ?, ?)
         ON DUPLICATE KEY UPDATE
            umlagekosten_chf = VALUES(umlagekosten_chf),
            active = VALUES(active)`,
        [year, month, meterName, bezeichnung, umlagekostenChf, !!active],
    );
...
/**
 * Also returns every active STANDING DEFAULT row (reading_year=0, reading_month=0 -
 * see upsertMeterUmlagekosten's docblock) regardless of the period, alongside whatever
 * period-matching explicit rows exist. The caller's aggregation
 * (lib/report.js aggregateMeterRowsByMonth()) applies a default to every real month in
 * range for that meter+Bezeichnung, UNLESS an explicit row for that exact month/meter/
 * Bezeichnung is also present, in which case the explicit one wins.
 *
 * @param {import('mariadb').Pool} pool
 * @param {string} fromDate "YYYY-MM-DD" inclusive
 * @param {string} toDate "YYYY-MM-DD" inclusive
...
async function queryMeterUmlagekostenPeriod(pool, fromDate, toDate) {
    ...
    return pool.query(
        `SELECT * FROM meter_umlagekosten
         WHERE active = TRUE
           AND (
             (reading_year = 0 AND reading_month = 0)
             OR (
               (reading_year > ? OR (reading_year = ? AND reading_month >= ?))
