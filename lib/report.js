 ...
 *   active) - itemized flat monthly costs added on top of the energy-based total. Passing
 *   nothing keeps prior behaviour (no Umlagekosten line/columns are populated).
...
function aggregateMeterRowsByMonth(meterRows, umlagekostenRows = []) {
    ...
    for (const u of umlagekostenRows) {
        ...
        if (!g) {
            ...
            // (e.g. set for a period with no meter_daily data yet) - nothing to attach it
            // to in this report; silently skipping here matches how energy rows are
            // already silently skipped for months with no data, not a new omission.
            continue;
        }
        g.umlagekostenItems.push({ bezeichnung: u.bezeichnung, betrag: Number(u.umlagekosten_chf) });
    }
